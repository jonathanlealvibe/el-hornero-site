...
// En el celular la ventana del asistente ocupa toda la pantalla y tapaba el botón: por eso el
// botón vive en la "capa superior" del navegador (popover manual), por encima del asistente.
export default function WhatsAppLinks() {
  const ref = useRef(null)
  useEffect(() => {
    const el = ref.current
    if (!el || typeof el.showPopover !== 'function') return
    const show = () => { try { if (!el.matches(':popover-open')) el.showPopover() } catch { /* sin soporte */ } }
    show()
    const t = setInterval(show, 1500)
    return () => { clearInterval(t); try { el.hidePopover() } catch { /* ignore */ } }
  }, [])
  return (
